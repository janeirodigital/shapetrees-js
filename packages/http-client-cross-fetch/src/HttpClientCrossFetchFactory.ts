 ...
 * The HttpClientCrossFetch uses the java.net.http library to implement `HttpClient`.
 ...
 */
export class HttpClientCrossFetchFactory implements HttpClientFactory, ExternalDocumentLoader {
   ...
   *   HttpClientFactoryManager.setFactory(new HttpClientCrossFetchFactory(...));
   *
   * @param useShapeTreeValidation
   * @return a new or existing java.net.http HttpClient
   * @throws ShapeTreeException if the HttpClientCrossFetch constructor threw one
   */
  public get(useShapeTreeValidation: boolean): HttpClientCrossFetch /* throws ShapeTreeException */ {
    try {
      return new HttpClientCrossFetch(this.useSslValidation, useShapeTreeValidation);
    } catch (ex: any) {
   ...
   *   DocumentLoaderManager.setLoader(new HttpClientCrossFetchFactory(...));
   *
